  ...
  $.fn.scannerDetection = function (options) {
    ...
    this.each(function () {
      ...
      self.isFocusOnIgnoredElement = function () {
        ...
        if (
          typeof options.ignoreIfFocusOn === "object" &&
          options.ignoreIfFocusOn.length
        ) {
          var focused = $(":focus");
      ...
      self.scannerDetectionTest = function (s) {
        ...
        if (
          stringWriting.length >= options.minLength &&
          lastCharTime - firstCharTime <
            stringWriting.length * options.avgTimeByChar
        ) {
          if (
            options.onScanButtonLongPressed &&
            scanButtonCounter > options.scanButtonLongPressThreshold
          )
            options.onScanButtonLongPressed.call(
              self,
              stringWriting,
              scanButtonCounter
            );
      ...
      $self
        ...
        .bind("keypress.scannerDetection", function (e) {
          ...
          if (firstCharTime && options.endChar.indexOf(e.which) !== -1) {
            ...
            !firstCharTime &&
            options.startChar.indexOf(e.which) !== -1
          ) {
            e.preventDefault();
            e.stopImmediatePropagation();
            callIsScanner = false;
