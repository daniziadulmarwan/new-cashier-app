...
ipcRenderer.on("load:tbody-tr", (e, content, titleBar, buyerInfo) => {
  ...
  switch (title) {
    case "checkout":
      $("#btn-submit").html('<i class="bi bi-printer"></i> Print');
      inputTotalSales = IMask(document.getElementById("total-sales"), {
        mask: "num",
        blocks: {
          num: {
            mask: Number,
      ...
      inputTotalReceived = IMask(document.getElementById("total-received"), {
        mask: "num",
        blocks: {
          num: {
            mask: Number,
...
const newDiscountFinal = () => {
  ...
  let newTotalDiscountFinal =
    parseFloat(discount_percent / 100) * parseFloat(total_sales) +
    parseFloat(discount_money);
  $("#total-discount-final").val(newTotalDiscountFinal);
};

const newTotal = (data_id) => {
  ...
  let newTotal;
  if (qty != "") {
    // Total Barang Dikurangi Diskon Uang
    newTotal =
      parseFloat(prd_price) * parseInt(qty) -
      (parseFloat(discount_percent) / 100) *
        parseFloat(prd_price) *
        parseInt(qty) -
      parseFloat(discount_money);
  } else {
    // Total Barang Dikurangi Diskon Uang
...
const submitUpdateSales = () => {
  ...
  $("#data")
    .find(".input-cost-of-product")
    .each(function () {
      let input_cost_of_prd = $(this).val();
      cost_of_product.push(input_cost_of_prd);
    });

  $("#data")
    .find(".input-prd-price")
    .each(function () {
      let prd_price = $(this).val();
      price.push(prd_price);
    });
  ...
  for (let i = 0; i < id.length; i++) {
    values.push(
      `('${id[i]}', '${input_date[i]}', '${sales_number}', '${buyer}', '${buyer_id}', '${payment}', '${description}', '${po_number}', '${due_date}', '${term}', '${sales_admin}', '${product_name[i]}', '${product_code[i]}', '${cost_of_product[i]}','${price[i]}','${qty[i]}','${unit[i]}','${discount_percent[i]}','${discount_money[i]}', '${total[i]}')`
    );
  }
    ...
    (err, rows) => {
      ...
      if (rows.length < 1) {
        values = `(datetime(
          "now",
          "localtime"
        ), '${sales_number}', '${discount_percent}', '${discount_money}', '${total_discount_final}')`;
        db.run(
          `insert into discount_final(input_date, invoice_number, discount_percent, discount_money, total_discount_final) values${values}`,
          (err) => {
            if (err) throw err;
            ipcRenderer.send("update-success:sales-edit");
          }
        ...
        values = `('${rows[0].id}',datetime(
          "now",
          "localtime"
        ), '${sales_number}', '${discount_percent}', '${discount_money}', '${total_discount_final}')`;
        db.run(
          `replace into discount_final(id, input_date, invoice_number, discount_percent, discount_money, total_discount_final) values${values}`,
          (err) => {
            if (err) throw err;
            ipcRenderer.send("update-success:sales-edit");
          }
