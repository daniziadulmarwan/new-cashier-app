...
const getProductName = () => {
  ...
  db.all(query, (err, rows) => {
    if (err) throw err;
    let option = '<option hidden value="">Nama Produk</option>';
    rows.forEach((row) => {
      option += `<option value="${row.product_name}">${row.product_name}</option>`;
    });
...
const openSales = () => {
  ...
  if (tax_checked.length < 1) {
    ...
    db.all(`select * from tax where id = 1`, (err, rows) => {
      if (err) throw err;
      tax_rate = parseFloat(rows[0].percentage) / 100;
      $("#input-tax").val(tax_rate);
    });
  }
  ...
  if (prd_name == "") {
    ...
    db.all(query, (err, rows) => {
      ...
      if (rows.length < 1) {
        ...
        $("#product_code").val(rows[0].product_code);
      }
    });
  }
};

const loadSales = (sales_num) => {
    ...
    if (rows.length < 1) {
      ...
      rows.map((row) => {
        ...
        if (discount_percent == "" && discount_money == "") {
          ...
          discount_info = `${discount_percent}%+${numberFormat(
            discount_money
          )}`;
        } else if (discount_percent == "" && discount_money != "") {
...
// START: TotalSales
const totalSales = (sales_number) => {
  let query = `select sum(total) as total_sales from sales where invoice_number = '${sales_number}'`;
  db.all(query, (err, rows) => {
    if (err) throw err;
    let total_sales = rows[0].total_sales;
    db.all(
      `select * from discount_final where invoice_number= '${sales_number}'`,
      (err, rows) => {
        if (err) throw err;
        if (rows.length < 1) {
          db.all(
            `select total_tax from sales_tax where invoice_number = '${sales_number}'`,
            (err, rows) => {
              if (err) throw err;
              if (rows.length < 1) {
                $("#total-and-tax").html(numberFormat(total_sales));
              } else {
                let total_tax = rows[0].total_tax;
                let net_total_sales =
                  parseFloat(total_sales) + parseFloat(total_tax);
                $("#total-and-tax, #info-total-sales").html(
                  numberFormat(net_total_sales)
                );
          ...
          let discount_percent = rows[0].discount_percent;
          let discount_money = rows[0].discount_money;
          ...
          if (discount_percent == "" && discount_money == "") {
            ...
            discount_final_info = `${discount_percent}%+${numberFormat(
              discount_money
            )}`;
          } else if (discount_percent == "" && discount_money != "") {
          ...
          let total_discount_final = rows[0].total_discount_final;
          db.all(
            `select total_tax from sales_tax where invoice_number = '${sales_number}'`,
            (err, rows) => {
              if (err) throw err;
              if (rows.length < 1) {
                let net_total_sales =
                  parseFloat(total_sales) - parseFloat(total_discount_final);
                ...
                let net_total_sales =
                  ...
                  parseFloat(total_discount_final) +
...
//START: SalesTax
const salesTax = (sales_number, tax_rate) => {
  db.all(
    `select sum(total) as total_sales from sales where invoice_number = '${sales_number}'`,
    (err, rows) => {
      if (err) throw err;
      if (rows.length < 1) {
        console.log(rows);
        ...
        let total_sales = rows[0].total_sales;
        db.all(
          `select total_discount_final from discount_final where invoice_number = '${sales_number}'`,
          (err, rows) => {
            if (err) throw err;
            if (rows.length < 1) {
              ...
              let total_discount_final = rows[0].total_discount_final;
              let total_sales_after_disc =
                parseFloat(total_sales) - parseFloat(total_discount_final);
              let total_tax =
                parseFloat(tax_rate) * parseFloat(total_sales_after_disc);
              queryTax(sales_number, total_tax);
            }
          }
        );
...
// START: QueryTax
const queryTax = (sales_number, total_tax) => {
  db.all(
    `select * from sales_tax where invoice_number = '${sales_number}'`,
    (err, rows) => {
      if (err) throw err;
      if (rows.length < 1) {
        db.run(
          `insert into sales_tax(input_date, invoice_number, total_tax) values (datetime('now', 'localtime'), '${sales_number}', '${total_tax}')`,
          (err) => {
            if (err) throw err;
        ...
        db.run(
          `update sales_tax set input_date = datetime('now', 'localtime'), total_tax = '${total_tax}' where invoice_number = '${sales_number}'`,
          (err) => {
            if (err) throw err;
            $("#tax").html(numberFormat(total_tax));
            totalSales(sales_number);
...
const insertSales = () => {
  ...
  let sql = `select * from products where product_code = '${product_code}' or barcode = '${product_code}'`;
  if (product_code != "" && product_code != null) {
    db.all(sql, (err, rows) => {
      ...
      if (rows.length < 1) {
        let alert = dialog.showMessageBoxSync({
          title: "Alert",
          type: "info",
          message: "Produk dengan kode " + product_code + " tidak ditemukan",
        });
        if (alert == 0) {
          $("#product_code").val("");
        }
      } else {
        let prd_name = rows[0].product_name;
        let prd_code = rows[0].product_code;
        let prd_price = rows[0].selling_price;
        let prd_cost = rows[0].cost_of_product;
        let unit = rows[0].unit;
        let discount_percent = "";
        let discount_money = "";

        db.all(
          `select * from sales where product_code = '${prd_code}' and invoice_number = '${sales_number}'`,
          (err, rows) => {
            if (err) console.log(err.message);
            if (rows.length < 1) {
              let qty = 1;
              ...
              db.run(
                `insert into sales (input_date,invoice_number,buyer,buyer_id,payment,description,po_number,due_date,term,sales_admin,product_name,product_code,cost_of_product,price,qty,unit,discount_percent,discount_money,total) values(datetime('now','localtime'), '${sales_number}', '${buyer}', '${buyer_id}', '${payment}', '${description}', '${po_number}', '${due_date}', '${term}', '', '${prd_name}', '${prd_code}', '${prd_cost}', '${prd_price}', '${qty}', '${unit}', '${discount_percent}', '${discount_money}', '${total}')`,
                (err) => {
                  if (err) throw err;
              ...
              if (rows[0].discount_percent != "") {
                disc_percent = rows[0].discount_percent;
              } else {
                disc_percent = 0;
              }

              if (rows[0].discount_money != "") {
                disc_money = rows[0].discount_money;
              } else {
                disc_money = 0;
              }
              ...
              db.run(
                `update sales set input_date=datetime('now', 'localtime'), qty='${new_qty}', total='${net_new_total}' where product_code = '${prd_code}'`,
                (err) => {
                  if (err) throw err;
                  loadSales(sales_number);
                  $("#product_code").val("");
    ...
    dialog.showMessageBoxSync({
      type: "info",
      title: "Alert",
      message: "Silahkan isi kode produk terlebih dahulu",
    });
  }
};
...
const salesModal = (title) => {
  ...
  if (sales_number != "") {
    db.all(
      `select * from sales where invoice_number = '${sales_number}'`,
      (err, rows) => {
        if (err) throw err;
        if (rows.length < 1) {
          dialog.showMessageBoxSync({
            ...
            type: "info",
            message:
              "Tidak terdapat record penjualan yg dapat diedit, silahkan masukkan record penjualan terlebih dahulu",
          });
        } else {
          ipcRenderer.send(
            "load:sales-modal",
